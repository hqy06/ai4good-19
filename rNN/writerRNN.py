...
def fit_model(writer_rnn, device, encoder, criterion, lr, optimizer, batch_size, in_batches, out_batches, n_epoch=3,
              verbose=False, save_model=False, print_every=1, save_every=2):
    iter_counter = 0
    total_loss = 0
    for epoch in range(n_epoch):
        ...
        if verbose and epoch % print_every == 0:
            print('Epoch {:>3}/{} | itr={:8}\tepoch_loss={:.5f} iter_loss={:.5f}'.format(epoch, n_epoch, iter_counter,
        ...
        if save_model and epoch % save_every == 0:
            ...
            file_path = os.path.join(
                'checkpoints', '{}-writer-{}.pth'.format(timestamp, iter_counter))
            torch.save(writer_rnn.state_dict(), file_path)
...
def predict(writer_rnn, device, encoder, decoder, seed, text_len, top_k=5):
    ...
    for w in seed:
        ...
        out, (hid_state, mem_state) = writer_rnn(
            in_text, (hid_state, mem_state))

    _, top_xs = torch.topk(out[0], k=top_k)
    choice = np.random.choice((top_xs.tolist())[0])

    text.append(decoder[choice])

    for i in range(text_len):
        in_text = torch.tensor([[choice]]).to(device)
        out, (hid_state, mem_state) = writer_rnn(
            in_text, (hid_state, mem_state))
        _, top_xs = torch.topk(out[0], k=top_k)
        choice = np.random.choice((top_xs.tolist())[0])
